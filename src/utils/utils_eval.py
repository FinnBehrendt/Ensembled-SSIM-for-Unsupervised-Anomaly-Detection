...
def _test_step(self, final_volume, data_orig, data_seg, data_mask, batch_idx, ID, label_vol) :
        ...
        # init
        SSIM_list = []
        for kernel in self.cfg.get('kernel_ens',[11]):
            for sigma in self.cfg.get('sigma_ens',[.3,.5,0.7,.9,1.1,1.3,1.5,1.7]): 
        ...
        for SSIM_ in SSIM_list: # average over all SSIMs
            ssim_agg, ssim = SSIM_(final_volume.type_as(data_orig),data_orig)
            ssims.append(ssim)
