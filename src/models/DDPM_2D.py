...
class DDPM_2D(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        _test_step(self, final_volume.float().cpu(), data_orig.float().cpu(), data_seg.float().cpu(), data_mask.float().cpu(), batch_idx, ID, label) # everything that is independent of the model choice
